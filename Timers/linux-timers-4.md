...
If you do not know or do not remember what is it a `possible` cpu, you can read the special [part](/Concepts/linux-cpu-2.md) of this book which describes `cpumask` concept in the Linux kernel. In short words, a `possible` processor is a processor which can be plugged in anytime during the life of the system.
...
The `tvec_bases` represents [per-cpu](/Concepts/linux-cpu-1.md) variable which represents main data structure for a dynamic timer for a given processor. This `per-cpu` variable defined in the same source code file:
...
* [cpumask](/Concepts/linux-cpu-2.md)
